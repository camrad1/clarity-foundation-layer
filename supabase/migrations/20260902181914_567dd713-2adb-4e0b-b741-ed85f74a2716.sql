-- Successful-result labels come straight from the WelcomeHome ActivityResults lookup payload.
CREATE INDEX IF NOT EXISTS idx_wh_activities_tour_scope
  ON public.wh_activities (organization_id, community_id, activity_type_id, completed_local_date);

CREATE OR REPLACE FUNCTION public.wh_successful_result_labels(_org_id uuid)
RETURNS text[]
LANGUAGE sql
STABLE SECURITY DEFINER
SET search_path TO 'public'
AS $$
  SELECT COALESCE(array_agg(l.label), ARRAY[]::text[])
...
CREATE OR REPLACE FUNCTION public.wh_tour_page(
  _org_id uuid, _start date, _end date,
  _community_ids uuid[] DEFAULT NULL::uuid[],
  _mode text DEFAULT 'successful',
 ...
 RETURNS TABLE(id uuid, source_id text, community_id uuid, prospect_source_id text,
               activity_type_label text, result_label text, successful boolean,
               first_completed_of_type boolean, completed_local_date date, total_count bigint)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  SELECT ac.id, ac.source_id, ac.community_id, ac.prospect_source_id,
         ...
         (ac.result_label = ANY(ok_labels)) AS successful,
   ...
   WHERE ac.organization_id = _org_id
     ...
     AND (mode = 'all' OR ac.result_label = ANY(ok_labels))
...
BEGIN
  ...
  a AS (
    SELECT ac.id, ac.activity_type_id, ac.user_id_source, ac.prospect_source_id,
           ...
           (ac.result_label = ANY(ok_labels)) AS ok
      FROM public.wh_activities ac
     WHERE ac.organization_id = _org_id
       AND ac.community_id = ANY(scope)
  ),
