ALTER TABLE public.wh_deposit_transactions ADD COLUMN IF NOT EXISTS deposit_type text;

UPDATE public.wh_deposit_transactions
   SET deposit_type = COALESCE(metadata->>'deposit_types_name', transaction_type),
       transaction_type = CASE WHEN is_refund IS TRUE THEN 'Refund' ELSE 'Deposit' END
 WHERE deposit_type IS DISTINCT FROM COALESCE(metadata->>'deposit_types_name', transaction_type)
    OR transaction_type IS DISTINCT FROM (CASE WHEN is_refund IS TRUE THEN 'Refund' ELSE 'Deposit' END);

CREATE INDEX IF NOT EXISTS idx_wh_deposits_kind
  ON public.wh_deposit_transactions (organization_id, community_id, transaction_type, deposit_type, occurred_local_date);

CREATE OR REPLACE FUNCTION public.wh_deposit_page(_org_id uuid, _start date, _end date, _community_ids uuid[] DEFAULT NULL::uuid[], _limit integer DEFAULT 50, _offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, source_id text, community_id uuid, prospect_source_id text, transaction_type text, deposit_type text, amount numeric, occurred_local_date date, total_count bigint)
 LANGUAGE plpgsql
...
BEGIN
  ...
  RETURN QUERY
  SELECT dt.id, dt.source_id, dt.community_id, dt.prospect_source_id,
         dt.transaction_type, dt.deposit_type, dt.amount, dt.occurred_local_date,
         count(*) OVER ()
   ...
   ORDER BY dt.occurred_local_date DESC, dt.source_id
