CREATE OR REPLACE FUNCTION public.wh_conversion_rates(_org_id uuid, _start date, _end date, _community_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'cohort', jsonb_build_object(
      ...
      'touredThenMovedIn', (SELECT count(*) FILTER (WHERE toured AND moved_in)::int FROM cf),
    ...
    'maturity', COALESCE((SELECT jsonb_agg(x ORDER BY ord) FROM (
        SELECT CASE WHEN age_days < 30 THEN 1 WHEN age_days < 60 THEN 2 WHEN age_days < 90 THEN 3 ELSE 4 END AS ord,
               jsonb_build_object(
                 ...
                 'touredThenMovedIn', count(*) FILTER (WHERE toured AND moved_in)::int,
    ...
    'byCommunity', COALESCE((SELECT jsonb_agg(x ORDER BY (x->>'inquiries')::int DESC) FROM (
        SELECT jsonb_build_object(
                 ...
                 'touredThenMovedIn', sum(toured_mi)::int,
...
CREATE OR REPLACE FUNCTION public.wh_conversion_series(_org_id uuid, _start date, _end date, _grain text DEFAULT 'month'::text, _community_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
