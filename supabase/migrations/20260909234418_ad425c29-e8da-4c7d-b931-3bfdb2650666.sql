...
DECLARE
  ...
  v_reopened integer;
BEGIN
  -- Recover chunks abandoned mid-run (worker timeout / restart).
...
SELECT cron.schedule('google-stalled-recovery', '25 * * * *', $$UPDATE public.google_backfill_chunks SET status='pending', last_error='recovered from stalled run' WHERE status='running' AND coalesce(started_at, created_at) < now() - interval '30 minutes';$$);