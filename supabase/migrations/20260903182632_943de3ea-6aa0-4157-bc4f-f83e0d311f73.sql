...
BEGIN
  ...
  -- MONTH ROW.
  -- Finalized month-end occupancy only exists after the calendar month ends.
  -- During an active month the row is month-to-date: event metrics keep their
  -- validated definitions and accumulate MTD, while every finalized occupancy
  -- field stays NULL so the UI renders "—". Current occupancy lives in the
  -- Current Weekly Summary; the forward-looking view lives in the projected
  -- month-end fields below.
