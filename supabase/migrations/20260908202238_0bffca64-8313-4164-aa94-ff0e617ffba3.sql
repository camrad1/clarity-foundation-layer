    ...
    SELECT pr.community_id AS cid,
           date_trunc('month',
             (CASE s.inquiry_date_field
              ...
              AT TIME ZONE COALESCE(c.timezone, 'UTC'))::date)::date AS m,
           count(*)::int AS n
     ...
     WHERE pr.organization_id = _org_id
       ...
       AND ((CASE s.inquiry_date_field
               ...
               WHEN 'active_at' THEN pr.active_at
               ELSE pr.created_at_source END
             AT TIME ZONE COALESCE(c.timezone, 'UTC'))::date) BETWEEN p_start AND p_end
     GROUP BY 1, 2
  ),
  tr AS (
...
REVOKE ALL ON FUNCTION public.community_trend_matrix(uuid, date, integer, uuid[]) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.community_trend_matrix(uuid, date, integer, uuid[]) FROM anon;
GRANT EXECUTE ON FUNCTION public.community_trend_matrix(uuid, date, integer, uuid[]) TO authenticated;
GRANT EXECUTE ON FUNCTION public.community_trend_matrix(uuid, date, integer, uuid[]) TO service_role;